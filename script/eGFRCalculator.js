...
document.getElementById('egfr-form').addEventListener('submit', function(event) {
    ...
    if (isNaN(age) || isNaN(creatinine) || !sex || !ethnicity) {
        ...
        eGFR = calculateEGFR(sex, age, creatinine, ethnicity );
        displayeGFRresult(eGFR); // diplays the last result of the reading
        updateChart(eGFR); // Update the chart with the new value
